...
workspace_dir = '/kaggle/input/food11/food11'#定义需要读取的路径
print("Reading data")
...
train_transform = transforms.Compose([
    ...
    #transforms.Normalize((0.5,0.5,0.5),(0.5,0.5,0.5))#对图像进行归一化，使得每个通道的均值为0.5，标准差为0.5
])
...
class Classifier(nn.Module):
    def __init__(self):
        ...
        self.cnn = nn.Sequential(
            # ----------------  第一个卷积层+最大池化层  ------------------#
            nn.Conv2d(3, 64, 3, 1, 1), #第一个卷积层，输入通道数为3，输出通道数为64，卷积核大小为3x3，步长为1，填充为1
            nn.BatchNorm2d(64),# 批标准化层，用于对卷积层的输出进行归一化
            nn.ReLU(), # ReLU激活函数
            nn.MaxPool2d(2, 2, 0),# 最大池化层，池化核大小为2x2，步长为2
            # ----------------  第二个卷积层+最大池化层  ------------------#
            nn.Conv2d(64, 128, 3, 1, 1),#第二个卷积层，输入通道数为64，输出通道数为128，卷积核大小为3x3，步长为1，填充为1
            nn.BatchNorm2d(128),# 批标准化层，用于对卷积层的输出进行归一化
            nn.ReLU(),# ReLU激活函数
            nn.MaxPool2d(2, 2, 0),# 最大池化层，池化核大小为2x2，步长为2
            # ----------------  第三个卷积层+最大池化层  ------------------#
            nn.Conv2d(128, 256, 3, 1, 1),#第三个卷积层，输入通道数为128，输出通道数为256，卷积核大小为3x3，步长为1，填充为1
            nn.BatchNorm2d(256),#同上，以下不再解释
            nn.ReLU(),
            nn.MaxPool2d(2, 2, 0),
            # ----------------  第四个卷积层+最大池化层  ------------------#
            nn.Conv2d(256, 512, 3, 1, 1),#同上，以下不再解释
            nn.BatchNorm2d(512),
            nn.ReLU(),
            nn.MaxPool2d(2, 2, 0),
           ...
           # nn.Conv2d(256, 256, 3, 1, 1),
            ...
            # ----------------  第五个卷积层+最大池化层  ------------------#
            nn.Conv2d(512, 512, 3, 1, 1),#同上，以下不再解释
            nn.BatchNorm2d(512),
            nn.ReLU(),
            nn.MaxPool2d(2, 2, 0),
...
for epoch in range(num_epoch): 
    ...
    for i, data in enumerate(train_loader):#使用tqdm显示进度
...
for epoch in range(num_epoch):
    ...
    for i, data in enumerate(train_val_loader):#使用tqdm显示进度
        optimizer.zero_grad()# 梯度参数归零，清除之前的梯度信息
